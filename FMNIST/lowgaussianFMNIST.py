...
for i in range(9, 10):
    ...
    np.savetxt("lfactorfmaccuracies_" + str(i) + ".csv", res, delimiter=",")
    ...
    enr = outofsample(net)
    np.savetxt(X=enr, fname="lfactorentrofullfmm" + str(i) + ".csv", delimiter=",")
    enr = outofsample(net, True)
    np.savetxt(X=enr, fname="mpmlfactorentrofullfmm" + str(i) + ".csv", delimiter=",")
    torch.save(net.state_dict(), "lfactorffm" + str(i) + ".par")
    # pepochs = 1
    for epoch in range(pepochs):
        ptrain(net, optimizer, epoch, i)
    res = test_ensemble(net)
    np.savetxt("lfactorptfmmaccuracies_" + str(i) + ".csv", res, delimiter=",")
    enr = outofsample(net)
    np.savetxt(X=enr, fname="lfactorpentrofullfmm" + str(i) + ".csv", delimiter=",")
    enr = outofsample(net,True)
    np.savetxt(X=enr, fname="mpmlfactorpentrofullfmm" + str(i) + ".csv", delimiter=",")
    torch.save(net.state_dict(), "lfactorffmp" + str(i) + ".par")
