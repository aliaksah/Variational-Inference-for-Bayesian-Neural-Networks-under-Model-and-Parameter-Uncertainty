...
mnist_loader = torch.utils.data.DataLoader(
    datasets.MNIST('./mnist', train=False, download=True, transform=transforms.ToTensor()), batch_size=5,
    shuffle=False)
...
for i in range(9, 10):
    ...
    plt.show()
    plt.savefig('./results/hfmn1fg' + str(i) + '.png')
    plt.close()
    plt.hist(net.l2.alpha.view(-1).cpu().detach().numpy(), bins=100, color="skyblue", ec="skyblue")
    plt.show()
    plt.savefig('./results/hfmn2fg' + str(i) + '.png')
    plt.close()
    plt.hist(net.l3.alpha.view(-1).cpu().detach().numpy(), bins=100, color="skyblue", ec="skyblue")
    plt.show()
    plt.savefig('./results/hfmn3fg' + str(i) + '.png')
    plt.close()
    enr = outofsample(net)
    np.savetxt(X=enr, fname="entrofullfaussfmm" + str(i) + ".csv", delimiter=",")
    enr = outofsample(net, True)
    np.savetxt(X=enr, fname="mpmentrofullfaussfmm" + str(i) + ".csv", delimiter=",")
    torch.save(net.state_dict(), "fgauusfm" + str(i) + ".par")
    # pepochs = 1
    for epoch in range(pepochs):
        ptrain(net, optimizer, epoch, i)
    res = test_ensemble(net)
    np.savetxt("ptgaussfmmaccuracies_" + str(i) + ".csv", res, delimiter=",")
    enr = outofsample(net)
    np.savetxt(X=enr, fname="gausspentrofullfmm" + str(i) + ".csv", delimiter=",")
    enr = outofsample(net, True)
    np.savetxt(X=enr, fname="mpmgausspentrofullfmm" + str(i) + ".csv", delimiter=",")
    torch.save(net.state_dict(), "gaussffmp" + str(i) + ".par")
