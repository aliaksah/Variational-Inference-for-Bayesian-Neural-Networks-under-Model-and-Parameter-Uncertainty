...
for i in range(0, 10):
    ...
    np.savetxt("hypermaccuraciesfm_" + str(i) + ".csv", res, delimiter=",")
    ...
    plt.show()
    plt.savefig('./results/hyperhmn1fg' + str(i) + '.png')
    plt.close()
    plt.hist(net.l2.alpha.view(-1).cpu().detach().numpy(), bins=100, color="skyblue", ec="skyblue")
    plt.show()
    plt.savefig('./results/hyperhmn2fg' + str(i) + '.png')
    plt.close()
    plt.hist(net.l3.alpha.view(-1).cpu().detach().numpy(), bins=100, color="skyblue", ec="skyblue")
    plt.show()
    plt.savefig('./results/hyperhmn3fg' + str(i) + '.png')
    plt.close()

    enr = outofsample(net)
    np.savetxt(X=enr, fname="hyperentrofullmmfm" + str(i) + ".csv", delimiter=",")
    enr = outofsample(net)
    np.savetxt(X=enr, fname="mpmhyperentrofullmmfm" + str(i) + ".csv", delimiter=",")
    torch.save(net.state_dict(), "hyperfmfm" + str(i) + ".par")
    # pepochs = 1
    for epoch in range(pepochs):
        ptrain(net, optimizer, epoch, i)
    res = test_ensemble(net)
    np.savetxt("hyperptmmaccuraciesfm_" + str(i) + ".csv", res, delimiter=",")
    enr = outofsample(net)
    np.savetxt(X=enr, fname="hyperpentrofullmmfm" + str(i) + ".csv", delimiter=",")
    enr = outofsample(net, True)
    np.savetxt(X=enr, fname="mpmhyperpentrofullmmfm" + str(i) + ".csv", delimiter=",")
    torch.save(net.state_dict(), "hyperfmpfm" + str(i) + ".par")
