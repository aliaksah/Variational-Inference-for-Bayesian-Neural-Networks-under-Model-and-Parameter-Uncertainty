...
class Net(nn.Module):
    def __init__(self):
        ...
        self.relu = nn.ReLU()
        # layers
        self.fc1 = LinearGroupNJ(28 * 28, 400, clip_var=0.04)
        self.fc2 = LinearGroupNJ(400, 600)
        self.fc3 = LinearGroupNJ(600, 10)
...
for i in range(9,10):
    ...
    np.savetxt("fmnisthorsepfull_" + str(i) + ".csv", res, delimiter=",")
    torch.save(net.state_dict(), "horseffmfull" + str(i) + ".par")

    enr = outofsample(net)
    np.savetxt(X=enr, fname="entrohorsefmm" + str(i) + ".csv", delimiter=",")

    layers = [net.fc1, net.fc2, net.fc3]
    thresholds = [-2.8, -3., -5.]
    masks = net.get_masks(thresholds)
    crs = compute_compression_rate(layers, masks)
    ...
    torch.save(net.state_dict(), "horsefmreduced" + str(i) + ".par")
    enr = outofsample(net)
    np.savetxt(X=enr, fname="entrohorseprunedmm" + str(i) + ".csv", delimiter=",")
    np.savetxt("mnisthorseprunded_" + str(i) + ".csv", res, delimiter=",")

    for epoch in range(50):
        posttrain(net, optimizer, epoch, i, layers=layers, masks=masks)

    res = test_ensemble(net)
    torch.save(net.state_dict(), "horsefmreducedpost" + str(i) + ".par")
    enr = outofsample(net)
    np.savetxt(X=enr, fname="entrohorseprunedpostmm" + str(i) + ".csv", delimiter=",")
    np.savetxt("mnisthorseprundedpost_" + str(i) + ".csv", res, delimiter=",")
