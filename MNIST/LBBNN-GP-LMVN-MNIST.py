...
for i in range(0, 10):
    ...
    plt.show()
    plt.savefig('./results/hmn1fg' + str(i) + '.png')
    plt.close()
    plt.hist(net.l2.alpha.view(-1).cpu().detach().numpy(), bins=100, color="skyblue", ec="skyblue")
    plt.show()
    plt.savefig('./results/hmn2fg' + str(i) + '.png')
    plt.close()
    plt.hist(net.l3.alpha.view(-1).cpu().detach().numpy(), bins=100, color="skyblue", ec="skyblue")
    plt.show()
    plt.savefig('./results/hmn3fg' + str(i) + '.png')
    plt.close()
    enr = outofsample(net)
    np.savetxt(X=enr, fname="entrofullfaussmm" + str(i) + ".csv", delimiter=",")

    enr = outofsample(net, True)
    np.savetxt(X=enr, fname="mpmentrofullfaussmm" + str(i) + ".csv", delimiter=",")

    torch.save(net.state_dict(), "fgauusm" + str(i) + ".par")
    # pepochs = 1
    for epoch in range(pepochs):
        ptrain(net, optimizer, epoch, i)
    res = test_ensemble(net)
    np.savetxt("ptgaussmmaccuracies_" + str(i) + ".csv", res, delimiter=",")
    enr = outofsample(net)
    np.savetxt(X=enr, fname="gausspentrofullmm" + str(i) + ".csv", delimiter=",")

    enr = outofsample(net, True)
    np.savetxt(X=enr, fname="mpmgausspentrofullmm" + str(i) + ".csv", delimiter=",")

    torch.save(net.state_dict(), "gaussfmp" + str(i) + ".par")
